...
def get_texts_from_jsonl(jsonl_path, doc_ids):
    """
    Given a path to a JSONL file and a list of document IDs,
    return a list of `text` fields matching those IDs.
    """
    ...
    with open(jsonl_path, "r") as f:
        for line in f:
            doc = json.loads(line)
            if doc["_id"] in doc_id_set:
                found_texts.append(doc["text"])

    return found_texts

def query_llm_with_extra_context(query, extra_text):
...
def run_pipeline(user_query: str):
    ...
    print("!!!!!!!")
    #print(similar_nodes)
    main_node = similar_nodes[0][0]  # (node, score)
    document_id = main_node.get("documentId", "unknown")
    ...
    return final_answer, document_id
...
def log_mismatch(question, pipeline_output, graph_output, correct_anwer, Incorrect_anwer):
    with open("after_seminar_small_kg/logs/23_04_previous_kg_mismatch_log.txt", "a") as log_file:
...
def main():
    ...
    for iter in range(args.repeat_times):
        ...
        for i in target_queries_idx:
            ...
            if args.use_truth == 'True':
                ...
                try:
                    ...
                    graph_output, top_document_id = run_pipeline(question)
                    print(top_document_id)
                    
                    #graph_output = query_llm_with_context(question, matched_results)
                    # # Step 2: If the LLM says "Unknown", try again with extra document context
                    if graph_output.strip().lower() == "not in knowledge base.":
                        top_doc_texts = get_texts_from_jsonl("datasets/nq/combined_by_title_from after_seminar_1.jsonl", top_document_id)
                        graph_output = query_llm_with_extra_context(question, top_doc_texts)
                ...
                final_output=""
                if clean_str(incorrect_answers[i]['correct answer']) in clean_str(graph_output):
                    final_output=graph_output
                else:
                    final_output=rag_response
                    
                if clean_str(incco_ans) in clean_str(final_output):
                    asr_cnt += 1  
                    print("asr_count:",asr_cnt)
                else:
                    print("Correct info found.")    
                log_mismatch(question, rag_response, graph_output,incorrect_answers[i]['correct answer'],incco_ans)
        ...
        print(f'Saving iter results to mylogs/query_results/{args.query_results_dir}/{args.name}.json')
